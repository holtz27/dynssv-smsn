...
dic = function(data, mu_t, sigma_t){
  ...
  # 1. Log-likelihood for each iteration and time
  ...
  # 2. Average deviance: \bar{D}
  D_bar = -2 * mean(rowSums(loglik_matrix))
  
  # 3. Average parameters: \hat{\mu}_t, \hat{\sigma}_t
  mu_hat     = colMeans(mu_t)
  sigma_hat  = colMeans(sigma_t)
  
  # 4. Deviance in the posterior mean: D(\hat{\theta})
  D_hat = -2 * sum(dnorm(data,
                         mean = mu_hat,
                         sd   = sigma_hat,
                         log  = TRUE))
  # 5. Penalization for effective complexity
  p_D = D_bar - D_hat
  # 6. Final calculation of DIC
  dic = D_bar + p_D
